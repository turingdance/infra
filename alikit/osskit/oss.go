...
type PolicyOption func(policyToken *PolicyToken)

const PREFIXXOSSCALLBACK = "x-oss-callback-"

func XvarOption(k string, v interface{}) PolicyOption {
	return func(policyToken *PolicyToken) {
		if policyToken.Xvar == nil || len(policyToken.Xvar) == 0 {
			policyToken.Xvar = map[string]interface{}{}
		}
		if strings.HasPrefix(k, PREFIXXOSSCALLBACK) {
			policyToken.Xvar[k] = v
		} else {
			policyToken.Xvar[PREFIXXOSSCALLBACK+k] = v
		}
	}
}
func GeneratePolicy(conf OssConf, options ...PolicyOption) (policyToken *PolicyToken, err error) {
	now := time.Now().Unix()
